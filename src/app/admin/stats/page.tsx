...
function buildStatsCsvContent(stats: StatsResponse): string {
  ...
  lines.push(`${escapeCsvCell("참여현황")},${escapeCsvCell("구조 경채")},${escapeCsvCell(stats.byExamType.CAREER_RESCUE)}`);
  lines.push(`${escapeCsvCell("참여현황")},${escapeCsvCell("소방학과 경채")},${escapeCsvCell(stats.byExamType.CAREER_ACADEMIC)}`);
  lines.push(`${escapeCsvCell("참여현황")},${escapeCsvCell("구급 경채")},${escapeCsvCell(stats.byExamType.CAREER_EMT)}`);
  ...
  lines.push("지역별 통계,,,,,,,");
  lines.push("지역,공채,구조 경채,소방학과 경채,구급 경채,합계,평균 원점수,평균 최종점수");
  ...
  for (const row of stats.byRegionPrediction) {
    lines.push(
      [
        row.regionName,
        row.examType === "PUBLIC" ? "공채" : row.examType === "CAREER_RESCUE" ? "구조 경채" : row.examType === "CAREER_ACADEMIC" ? "소방학과 경채" : "구급 경채",
  ...
  if (stats.difficulty?.subjects?.length) {
    for (const row of stats.difficulty.subjects) {
      const name = row.examType === "CAREER_RESCUE"
        ? `${row.subjectName}(구조 경채)`
        : row.examType === "CAREER_ACADEMIC"
          ? `${row.subjectName}(소방학과 경채)`
...
export default function AdminStatsPage() {
  ...
  const availablePredictionExamTypes = useMemo<PredictionExamType[]>(() => {
    ...
    const hasCareerRescue = rows.some((item) => item.examType === "CAREER_RESCUE");
    const hasCareerAcademic = rows.some((item) => item.examType === "CAREER_ACADEMIC");
    const hasCareerEmt = rows.some((item) => item.examType === "CAREER_EMT");

    const next: PredictionExamType[] = [];
    if (hasPublic) next.push("PUBLIC");
    if (hasCareerRescue) next.push("CAREER_RESCUE");
    if (hasCareerAcademic) next.push("CAREER_ACADEMIC");
    if (hasCareerEmt) next.push("CAREER_EMT");
      ...
      {isLoading ? (
        ...
        <>
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-500">소방학과 경채</p>
              ...
              <table className="min-w-[960px] w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-3">소방학과 경채</th>
              ...
              <div className="inline-flex rounded-md border border-slate-200 bg-slate-100 p-1">
                {availablePredictionExamTypes.map((examType) => {
                  ...
                  return (
                    ...
                    >
                      {examType === "PUBLIC" ? "공채" : examType === "CAREER_RESCUE" ? "구조 경채" : examType === "CAREER_ACADEMIC" ? "소방학과 경채" : "구급 경채"}
                    </button>
                  );
                })}
              </div>
          ...
          <section className="space-y-3">
            ...
            {!stats.difficulty || stats.difficulty.totalResponses < 1 ? (
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-500">
                  * 경채 전용 과목은 해당 경채 유형 응시자 응답만 집계합니다.
                </p>
              </div>
            )}
          </section>
