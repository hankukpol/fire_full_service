...
export default function ForgotPasswordPage() {
  ...
  const [requestDebugUrl, setRequestDebugUrl] = useState("");
  ...
  const handleRequestSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!input) {
      setRequestError("이메일 또는 연락처를 입력해 주세요.");
      return;
    }
    ...
    try {
      ...
      const data = (await response.json()) as RequestResetResponse;

      if (!response.ok) {
        const message = data.error ?? "비밀번호 재설정 요청 처리 중 오류가 발생했습니다.";
        setRequestError(message);
        showErrorToast(message);
        return;
      }

      const message = data.message ?? "재설정 안내를 전송했습니다.";
      setRequestMessage(message);
      setRequestDebugUrl(typeof data.debugResetUrl === "string" ? data.debugResetUrl : "");
      showToast(message, "success");
    } catch {
      const message = "비밀번호 재설정 요청 처리 중 오류가 발생했습니다.";
      setRequestError(message);
      showErrorToast(message);
    } finally {
  ...
  const handleRecoverySubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!/^010-\d{4}-\d{4}$/.test(normalizedPhone)) {
      setRecoveryError("연락처는 010-XXXX-XXXX 형식으로 입력해 주세요.");
      return;
    }
      ...
      <Card className="w-full max-w-xl">
        <CardHeader>
          ...
          <p className="text-sm text-slate-500">
            이메일 재설정 링크 또는 복구코드로 비밀번호를 다시 설정할 수 있습니다.
          </p>
        </CardHeader>
        <CardContent className="space-y-8">
          <section className="space-y-4">
            <div>
              <h2 className="text-base font-semibold text-slate-900">이메일 재설정 링크 받기</h2>
              <p className="mt-1 text-sm text-slate-600">가입 시 등록한 이메일 또는 연락처를 입력해 주세요.</p>
            </div>
            <form className="space-y-3" onSubmit={handleRequestSubmit}>
              <div className="space-y-2">
                <Label htmlFor="identifier">이메일 또는 연락처</Label>
                <Input
                  id="identifier"
                  value={identifier}
                  onChange={(event) => setIdentifier(event.target.value)}
                  placeholder="name@example.com 또는 010-1234-5678"
                  required
                />
              </div>
              {requestError ? (
                <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">{requestError}</p>
              ) : null}
              {requestMessage ? (
                <div className="rounded-md bg-emerald-50 px-3 py-2 text-sm text-emerald-700">
                  <p>{requestMessage}</p>
                  {requestDebugUrl ? (
                    <a className="mt-1 inline-block underline" href={requestDebugUrl}>
                      개발환경 재설정 링크 열기
                    </a>
                  ) : null}
                </div>
          ...
          <section className="space-y-4 border-t border-slate-200 pt-6">
            <div>
              <h2 className="text-base font-semibold text-slate-900">복구코드로 비밀번호 재설정</h2>
              <p className="mt-1 text-sm text-slate-600">
                회원가입 시 저장한 복구코드 1개와 연락처를 입력해 주세요.
              </p>
            </div>
            ...
            {newRecoveryCodes.length > 0 ? (
              <div className="rounded-lg border border-amber-300 bg-amber-50 p-4">
                ...
                <p className="mt-1 text-xs text-amber-800">
                  아래 코드를 저장해 주세요. 페이지를 벗어나면 다시 확인할 수 없습니다.
                </p>
