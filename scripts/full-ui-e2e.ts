...
async function runDesktopUserFlow(userPage: Page): Promise<{ submissionId: number }> {
  ...
  for (let subjectIndex = 0; subjectIndex < subjectCount; subjectIndex += 1) {
    ...
    const { firstId, questionCount } = await fillCurrentSubjectQuickInputs(userPage);
    filledQuestions += questionCount;
  ...
  await userPage.waitForURL(/\/exam\/result\?submissionId=\d+/, { timeout: 20000 });

  const submissionIdParam = new URL(userPage.url()).searchParams.get("submissionId");
  const submissionId = Number(submissionIdParam);
  assertCondition(Number.isInteger(submissionId) && submissionId > 0, `Invalid submissionId in URL: ${userPage.url()}`);
